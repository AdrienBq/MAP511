...
def calc_mi(model, test_data_batch):
    ...
    for i in range(len(mu_batch_list)):
        ...
        # get z_samples
        ###############
        mu, logvar = mu_batch_list[i] , logvar_batch_list[i] 
        ...
        mu = torch.cat([mu_batch_list[_] for _ in indices], dim=0)
        logvar = torch.cat([logvar_batch_list[_] for _ in indices], dim=0)
        x_batch, nz = mu.size()
